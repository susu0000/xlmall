...
function T($template='',$layer=''){
    ...
    // 开发模块模板文件
    $tpl = strtolower(PLAYGROUND_PATH . $module . $layer . 's/' . $file . C('TMPL_TEMPLATE_SUFFIX'));
    if (is_file($tpl)) return $tpl;
    $tpl = strtolower(PLAYGROUND_PATH . $module . $layer . 's/' . end(explode('.', $file)) . C('TMPL_TEMPLATE_SUFFIX'));
    if (is_file($tpl)) return $tpl;
    // 应用模板文件
    $tpl = strtolower(APP_PATH . $module . $layer . 's/' . $file . C('TMPL_TEMPLATE_SUFFIX'));
    if (is_file($tpl)) return $tpl;
    $tpl = strtolower(APP_PATH . $module . $layer . 's/' . end(explode('.', $file)) . C('TMPL_TEMPLATE_SUFFIX'));
    if (is_file($tpl)) return $tpl;
    // 兼容模板文件
    $tpl = dirname($baseUrl) . '/base/' . str_replace($depr, '/', strtolower($file)) . C('TMPL_TEMPLATE_SUFFIX');
    if (is_file($tpl)) return $tpl;
    $tpl = dirname($baseUrl) . '/base/' . end(explode('.', $file)) . C('TMPL_TEMPLATE_SUFFIX');
    if (is_file($tpl)) return $tpl;
