	...
	private function ecjia_login()
	{
 	...
 	private function init_login(){
		if(isset($_GET["session"])){
			$openid=$_REQUEST["session"];
			$sql = 'SELECT user_name,user_id,email FROM ' . $GLOBALS['ecs']->table('users') . ' WHERE openid=\'' . $openid . '\' LIMIT 1';
			...
			if(!empty($row)){
		        ...
		        if($res['code']==0){
		        	$result=$res['result'];
		        	$username=time();
		        	$pass=md5($username,100);
		        	$email=$username.'@qq.com';
		        	$other=array(
		        		'nick_name'=>$result[0]['name'],
		        		'sex'=> $result[0]['sex'],
		        	    'user_picture'=>$result[0]['head'],
		        	    'openid' => $openid
		        	);
		        	
		        	$this->load_helper(array('passport'));
		        	register($username,$pass,$email,$other);
		        }
			}
