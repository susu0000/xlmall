...
class Editor extends \app\http\base\controllers\Backend
{
	...
	public function actionPreview()
	{
		...
		if (!empty($data)) {
			...
			\app\classes\Compile::setModule('preview', $data);
	...
	public function actionSave()
	{
		...
		if (!empty($data)) {
			$data = $this->transform($data);
			\app\classes\Compile::setModule('index', $data);
			$this->response(array('error' => 0, 'data' => $data));
		}
	...
	{
		if (\app\classes\Compile::cleanModule()) {
	...
	public function actionUpload()
	{
		...
		if ($res['error'] === 0) {
			...
			if (empty($album)) {
				$data = array('album_id' => 99, 'ru_id' => 0, 'album_mame' => '手机端可视化相册', 'sort_order' => 50, 'add_time' => gmtime());
				$this->db->table('gallery_album')->add($data);
			}

			$upinfo = $res['url']['file'];
			$data = array('pic_name' => $upinfo['name'], 'album_id' => 99, 'pic_file' => $upinfo['url'], 'pic_thumb' => '', 'pic_size' => $upinfo['size'], 'pic_spec' => $upinfo['name'], 'ru_id' => 0, 'add_time' => gmtime());
