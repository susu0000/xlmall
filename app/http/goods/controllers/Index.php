...
class Index extends \app\http\base\controllers\Frontend
{
	...
	public function actionIndex()
	{
		...
		$sql = 'SELECT shopNameSuffix FROM ' . $this->ecs->table('merchants_shop_information') . ' WHERE user_id = ' . $goods['user_id'];
		$shopNameSuffix = $this->db->getOne($sql);
		$this->assign('shopNameSuffix', $shopNameSuffix ? $shopNameSuffix : 0);
