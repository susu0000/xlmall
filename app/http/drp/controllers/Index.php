...
class Index extends Frontend {
    ...
    public function actionIndex() {
        // 是否已经成功开店
        $sql = "SELECT id,goods_id,shop_name FROM {pre}drp_shop WHERE user_id=" . $_SESSION['user_id'];
        ...
        $sql = "SELECT goods_id FROM {pre}drp_shop WHERE user_id=" . $_SESSION['user_id'];
    ...
    public function actionCategory() {
        ...
        if (IS_AJAX) {
            ...
            $show = $count > 0 ? 1 : 0;
            die(json_encode(array('goods_list' => $collection_goods['goods_list'],'totalPage' => $page_size)));
         }
        if (IS_POST) {
            $cateArr = I('post.cate');
            $cat_id = '';
