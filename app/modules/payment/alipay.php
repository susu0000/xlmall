...
class alipay
{
	public function get_code($order, $payment)
	{
		...
		$result = \ectouch\Http::doPost($gateway, $param . '&sign=' . md5($sign));
		...
		$param = substr($param, 0, -1);
		$sign = substr($sign, 0, -1) . $payment['alipay_key'];
		$button = '<a  type="button" class="box-flex btn-submit" onclick="javascript:_AP.pay(\'' . $gateway . $param . '&sign=' . md5($sign) . '\')">支付宝支付</a>';
		return $button;
	}
