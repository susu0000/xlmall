...
{
	$sql = 'select rw2.regionId, rw2.region_name from ' . $GLOBALS['ecs']->table('region_warehouse') . ' as rw1, ' . $GLOBALS['ecs']->table('region_warehouse') . ' as rw2' . ' where rw1.region_id = rw2.parent_id AND rw1.region_type = 0 order by rw2.regionId asc';
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
	...
	for ($i = 0; $i < count($warehouse_name); $i++) {
		if (!empty($warehouse_name[$i])) {
			...
			if (0 < $w_id) {
				...
				$sql = 'insert into ' . $GLOBALS['ecs']->table('warehouse_goods') . '(goods_id, region_id, region_number, warehouse_price, warehouse_promote_price, user_id, add_time)VALUES(\'' . $goods_id . '\',\'' . $warehouse_name[$i] . '\',\'' . intval($warehouse_number[$i]) . '\',\'' . floatval($warehouse_price[$i]) . '\',\'' . floatval($warehouse_promote_price[$i]) . '\',\'' . $user_id . '\',\'' . $add_time . '\')';
				$GLOBALS['db']->query($sql);
			}
		}
	}
...
{
	$sql = 'SELECT og.goods_id, g.goods_name, g.goods_thumb, og.goods_number, og.goods_price, og.goods_price, og.extension_code, og.goods_name AS extension_name FROM ' . $GLOBALS['ecs']->table('order_goods') . ' as og ' . 'left join ' . $GLOBALS['ecs']->table('goods') . ' as g on og.goods_id = g.goods_id ' . 'left join ' . $GLOBALS['ecs']->table('goods_activity') . ' as ga on og.goods_id = ga.act_id ' . 'WHERE og.order_id = \'' . $order_id . '\' and ga.review_status = 3 group by g.goods_id order by g.goods_id';
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
...
function get_goods_minMax_price($goods_id = 0, $warehouse_id = 0, $area_id = 0, $goods_price, $market_price, $type = 1)
{
	...
	if ($type == 1) {
		...
		$sql = 'SELECT ' . 'IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * \'' . $_SESSION['discount'] . '\') AS shop_price, ' . 'IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)) as promote_price, ' . 'g.promote_start_date, g.promote_end_date, g.is_promote ' . $goodsLeftJoin . 'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' . 'ON mp.goods_id = g.goods_id AND mp.user_rank = \'' . $_SESSION['user_rank'] . '\' ' . ' FROM ' . $GLOBALS['ecs']->table('goods') . ' as g ' . 'WHERE goods_id = \'' . $goods_id . '\'';
...
{
	$sql = 'select a.attr_id, a.attr_name from ' . $GLOBALS['ecs']->table('goods_attr') . ' as ga ' . ' left join ' . $GLOBALS['ecs']->table('attribute') . ' as a on ga.attr_id = a.attr_id ' . ' where goods_id = \'' . $goods_id . '\' group by a.attr_id';
	$attr_list = $GLOBALS['db']->getAll($sql);
...
function goodsShippingFee($goods_id = 0, $warehouse_id = 0, $region = array())
{
	...
	if (!empty($goodsInfo['freight'])) {
		if ($goodsInfo['freight'] == 1) {
			...
			$sprice = $goods_transport['sprice'];
		}

		$shippingInfo['shipping_fee_formated'] = price_format($sprice, false);
		$shippingInfo['is_shipping'] = 1;
	}
	else {
