...
function get_user_default($user_id = 0)
{
	...
	$sql = 'SELECT email, mobile_phone,sex, pay_points,nick_name, user_money, credit_line, last_login, is_validated, user_picture, rank_points,user_rank FROM ' . $GLOBALS['ecs']->table('users') . ' WHERE user_id = \'' . $user_id . '\'';
	$row = $GLOBALS['db']->getRow($sql);
	$info = array();
	...
	if (is_dir(APP_WECHAT_PATH)) {
		...
		$info['user_picture'] = get_image_path($row['user_picture']);
...
function get_shop_name($ru_id = 0, $type = 0)
{
	...
	$sql = 'SELECT concat(shoprz_brandName, shopNameSuffix) as shop_name, shoprz_brandName, shopNameSuffix, rz_shopName FROM ' . $GLOBALS['ecs']->table('merchants_shop_information') . ' WHERE user_id = \'' . $ru_id . '\'';
