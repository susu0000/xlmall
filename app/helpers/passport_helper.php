...
function register($username, $password, $email, $other = array())
{
	...
	if (!$GLOBALS['user']->add_user($username, $password, $email)) {
		...
		$other_key_array = array('aite_id', 'nick_name', 'sex', 'user_picture', 'msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone', 'parent_id','openid');
...
function send_pwd_email($uid, $user_name, $email, $code)
{
	...
	if (send_mail($user_name, $email, '找回密码', $content, 1)) {
		return true;
	}
	else {
		return false;
