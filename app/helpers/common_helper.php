...
function get_snatch_result($id)
{
	...
	if ($rec) {
		...
		$sql = "SELECT ext_info \" .\r\n               \" FROM " . $GLOBALS['ecs']->table('goods_activity') . ' WHERE act_id= \'' . $id . '\' AND review_status = 3 AND act_type=' . GAT_SNATCH . ' LIMIT 1';
...
function get_final_price($goods_id, $goods_num = '1', $is_spec_price = false, $spec = array(), $warehouse_id = 0, $area_id = 0, $type = 0, $presale = 0, $add_tocart = 1, $show_goods = 0)
{
	...
	$sql = 'SELECT IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * \'' . $discount . '\') AS shop_price, mp.user_price, ' . $shop_price . 'IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)) as promote_price, ' . ' g.promote_start_date, g.promote_end_date' . ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ON mp.goods_id = g.goods_id AND mp.user_rank = \'' . $user_rank . '\' ' . $leftJoin . ' WHERE g.goods_id = \'' . $goods_id . '\'' . ' AND g.is_delete = 0 LIMIT 1';
...
function get_package_info($id)
{
	...
	$now = gmtime();
	$sql = 'SELECT act_id AS id,  act_name AS package_name, goods_id , goods_name, start_time, end_time, act_desc, ext_info, user_id, activity_thumb ' . ' FROM ' . $GLOBALS['ecs']->table('goods_activity') . ' WHERE act_id=\'' . $id . '\' AND review_status = 3 AND act_type = ' . GAT_PACKAGE;
...
function cate_history($size, $page, $sort, $order, $warehouse_id = 0, $area_id = 0)
{
	...
	if (!empty($_COOKIE['ECS']['list_history'])) {
		...
		$sql = 'SELECT b.brand_name, g.goods_sn, g.brand_id, g.goods_id, g.goods_name, g.user_id, g.goods_thumb,g.sales_volume, g.user_id, ' . 'IFNULL(mp.user_price, IF(g.model_price < 1, g.shop_price, IF(g.model_price < 2, wg.warehouse_price, wag.region_price)) * \'' . $_SESSION['discount'] . '\') AS shop_price, ' . 'IF(g.model_price < 1, g.promote_price, IF(g.model_price < 2, wg.warehouse_promote_price, wag.region_promote_price)) as promote_price ' . ' FROM ' . $GLOBALS['ecs']->table('goods') . ' as g ' . ' left join ' . $GLOBALS['ecs']->table('brand') . ' as b' . ' on g.brand_id = b.brand_id ' . $leftJoin . 'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' . 'ON mp.goods_id = g.goods_id AND mp.user_rank = \'' . $_SESSION['user_rank'] . '\' ' . ' WHERE ' . $where . ' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 group by g.goods_id  ORDER BY ' . $sort . ' ' . $order;
