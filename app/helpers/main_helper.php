...
function get_wechat_user_info($id = 0)
{
	if (is_wechat_browser() && is_dir(APP_WECHAT_PATH)) {
		$sql = 'SELECT u.user_name, u.nick_name, u.user_picture, wu.headimgurl, wu.nickname FROM ' . $GLOBALS['ecs']->table('users') . 'AS u ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('wechat_user') . 'AS wu ' . ' ON u.user_id = wu.ect_uid ' . ' WHERE u.user_id = \'' . $id . '\' ';
	}
	else {
...
function get_affiliate()
{
	...
	else if (!empty($_COOKIE['ecshop_affiliate_uid'])) {
...
function set_ru_id()
{
	if (is_dir(APP_WECHAT_PATH) && !empty($_GET['ru_id'])) {
...
function get_ru_id()
{
	if (is_dir(APP_WECHAT_PATH) && !empty($_COOKIE['ectouch_ru_id'])) {
		$ectouch_ru_id = intval($_COOKIE['ectouch_ru_id']);

		if ($GLOBALS['db']->getOne('SELECT ru_id FROM ' . $GLOBALS['ecs']->table('admin_user') . ' WHERE ru_id = \'' . $ectouch_ru_id . '\' ')) {
			return $ectouch_ru_id;
		}
		else {
