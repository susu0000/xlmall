...
function get_user_orders($user_id, $num = 10, $page = 1, $status = 0)
{
	...
	foreach ($res as $key => $row) {
		...
		if ($row['order_status'] == OS_UNCONFIRMED) {
			$url = url('user/order/cancel', array('order_id' => $row['order_id']));
			$row['handler'] = '<a class="btn-default" href="javascript:void(0);" onclick="confirm_cancel(\'' . l('confirm_cancel') . '\', \''. $url .'\')">' . l('cancel') . '</a>';
		}
		...
		$ru_id = $row['ru_id'];
		$row['order_goods'] = order_goods($row['order_id']);
...
function get_user_bouns_list($user_id, $type = 0, $num = 10, $start = 0)
{
	...
	$sql = 'SELECT  u.bonus_sn, u.order_id, b.user_id, b.type_name, b.type_money, b.min_goods_amount, b.use_start_date, b.use_end_date ' . ' FROM ' . $GLOBALS['ecs']->table('user_bonus') . ' AS u ,' . $GLOBALS['ecs']->table('bonus_type') . ' AS b ' . ' WHERE  u.bonus_type_id = b.type_id AND u.user_id = \'' . $user_id . '\'';
